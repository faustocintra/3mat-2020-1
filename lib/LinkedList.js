...
class LinkedList {
   ...
   insert(element, position) {
      ...
      if(position === this.count || (this.count === 0 && position === 0)) this.push(element)
         ...
         let elemAnt = this.head // Começa do início, já é a posição 0
         
         // Percorrer lista ligada até uma posição antes da desejada
         for(let i = 1; i < position; i++) {
            elemAnt = elemAnt.next
         }
         console.log('elemAnt', elemAnt.element)
         // Criar o novo nodo
         let newNode = new Node(element)

         // 1) Apontar o novo nodo para o próximo elemento ao
         // da posição de inserção
         newNode.next = elemAnt.next

         // 2) Apontar o elemento da posição para o novo elemento
         elemAnt.next = newNode
